<xsl:stylesheet exclude-result-prefixes="php" version="1.0" xmlns:php="http://php.net/xsl"
 ...
 <xsl:variable name="size" select="1"/>
 ...
 <xsl:template match="/">
  <xsl:if test="$count>0">
   <table cellpadding="3" cellspacing="3" width="90%">
        ...
        <xsl:when test="$end >= $count">
         ...
         <a>
          <xsl:attribute name="href">
           <xsl:value-of select="$BASEURL"/>/fedora/repository/<xsl:value-of
            select="$thisPid"/>/-/<xsl:value-of select="$thisTitle"/>/<xsl:value-of select="$page - 1"/>
          </xsl:attribute>
          &lt;&lt;Prev
         </a>
        </xsl:when>
    ...
    <xsl:for-each select="s:sparql/s:results/s:result[position()>=$start and position() &lt;=$end]">
       ...
       <center>
        <img>
         <xsl:attribute name="src">
          <xsl:value-of select="$BASEURL"/>/fedora/repository/<xsl:value-of select="$PID"/>/JPG/JPG.jpg
         </xsl:attribute>
        </img>
        <a>
         <xsl:attribute name="href">
   ...
   <div STYLE="text-align: center;">
    <xsl:choose>
     ...
     <xsl:when test="$end >= $count">
      <xsl:value-of select="$start"/>-<xsl:value-of select="$count"/>
      of <xsl:value-of select="$count"/> <br/>
      <table><xsl:attribute name="align">center</xsl:attribute><tr><td>
       <a>
        <xsl:attribute name="href">
        ...
        <img>
         <xsl:attribute name="src">
          <xsl:value-of select="$BASEURL"/>/fedora/repository/ilives:bunbury-00<xsl:value-of select="$page - 2"/>/JPG/JPG.jpg
         </xsl:attribute>
     ...
     <xsl:when test="$start = 1">
      ...
      <a>
       ...
       <img>
        <xsl:attribute name="src">
         <xsl:value-of select="$BASEURL"/>/fedora/repository/ilives:bunbury-00<xsl:value-of select="$page"/>/JPG/JPG.jpg
        </xsl:attribute>
        <xsl:attribute name="height">110</xsl:attribute>
        <xsl:attribute name="width">85</xsl:attribute>
       </img><br/>
 ...
 <xsl:template match="s:result">
  ...
  <td valign="top" width="25%">
   <a>
    ...
    <img>
     <xsl:attribute name="src">
      <xsl:value-of select="$BASEURL"/>/fedora/repository/<xsl:value-of select="$PID"/>/JPG/JPQ.jpg
     </xsl:attribute>
     <xsl:attribute name="alt">
      <xsl:value-of disable-output-escaping="yes" select="$newTitle"/>
     </xsl:attribute>
    </img>
   </a>
   <br clear="all"/>
